//https://github.com/nkappler/ctxmenu
...
(function() {
    ...
    function generateMenuItem(item, parentOrEvent) {
        var _a;
        ...
        if (itemIsAction(item)) {
            li.addEventListener("click", (e)=>item.action(e,parentOrEvent?.selectedElement));
            return li;
        }
    ...
    function setPosition(container, parentOrEvent) {
        ...
        const zoomLevel = (window.outerWidth) / (window.innerWidth-8);
        Object.assign(container.style, {
            left: pos.x + "px",
            top: pos.y + "px",
            width: rect.width + "px",
    ...
    function getPosition(rect, pos) {
        ...
        const zoomLevel = (window.outerWidth) / (window.innerWidth-8);
        return {
            x: hdir === "r" ? pos.x + (rect.width/zoomLevel) > maxX ? maxX - (rect.width/zoomLevel) : pos.x : pos.x < minX ? minX : pos.x,
            y: vdir === "d" ? pos.y + (rect.height/zoomLevel) > maxY ? maxY - (rect.height/zoomLevel) : pos.y : pos.y < minY ? minY : pos.y + (rect.height/zoomLevel) > maxY ? maxY - (rect.height/zoomLevel) : pos.y
        };
    }
    ...
    var ContextMenu = function() {
        ...
        ContextMenu.prototype.attach = function(target, ctxMenu, _config) {
            ...
            var handler = function(e) {
                e.selectedElement = document.activeElement;
        ...
        ContextMenu.prototype.generateDOM = function(ctxMenu, parentOrEvent, attributes) {
            ...
            ctxMenu.forEach((function(item) {
                var li = generateMenuItem(item, parentOrEvent);
